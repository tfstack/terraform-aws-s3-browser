...
<head>
    <title id="pageTitle">AWS S3 Explorer</title>
    <meta charset="utf-8">
    ...
    <link rel="stylesheet" href="https://cdn.datatables.net/2.0.1/css/dataTables.bootstrap.min.css" integrity="sha512-VMR2J9d1Y/nEZh/4y/hpeGiyRmNpQFSFUgP9gqY3/2kLf3hFUdmdgkwfzazE3HllT7wa+pSH78lr4guyJXN9zQ==" crossorigin="anonymous">
    <style type="text/css">
        #wrapper {
            padding-left: 0;
        }
                ...
                <div class="panel panel-primary">
                    ...
                    <div class="panel-heading clearfix">
                        ...
                        <div class="btn-group pull-left">
                            ...
                            <div class="title pull-left">
                                <span id="headerTitle">AWS S3 Explorer</span>&nbsp;
                            </div>
                            <!-- Bucket breadcrumbs -->
                            <div class="pull-right">
                                <ul id="breadcrumb" class="breadcrumb pull-right">
                                    <li class="active">
                                        <a href="#">&lt;<span id="bucket">bucket</span>&gt;</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div id="navbuttons" class="pull-right">
                            <div>
                                ...
                                <div class="btn-group">
                                    <label class="btn">
                                        <input type="checkbox" id="hidefolders">&nbsp;
                                            <span id="hide-folders-text">Hide folders?</span>
                                    </label>
                                </div>
                                <!-- Folder/Bucket radio group -->
                                <div class="btn-group" data-toggle="buttons">
                                    <label class="btn btn-primary active" id="folder-objects" title="View all objects in folder">
                                        <i class="fa fa-angle-double-up"></i>
                                        <input type="radio" name="optionsdepth" value="folder" id="optionfolder" checked>&nbsp;<span id="folder-text">Folder</span>
                                    </label>
                                    <label class="btn btn-primary" id="bucket-objects" title="View all objects in bucket">
                                        <i class="fa fa-angle-double-down"></i>
                                        <input type="radio" name="optionsdepth" value="bucket" id="optionbucket">&nbsp;<span id="bucket-text">Bucket</span>
                                    </label>
                                </div>
                                <!-- Bucket selection -->
                    ...
                    <div class="panel-body">
                        <table class="table table-bordered table-hover table-striped" id="tb-s3objects">
                            <thead>
                                <tr>
                                    <th id="col-object">Object</th>
                                    <th id="col-folder">Folder</th>
                                    <th id="col-last-modified">Last Modified</th>
                                    <th id="col-timestamp">Timestamp</th>
                                    <th id="col-size">Size</th>
                                </tr>
                            </thead>
...
<script src="https://cdn.datatables.net/2.0.1/js/dataTables.min.js" integrity="sha512-PhxKYD+yO/lC78X6FH4UvD00aTR7qMJm3DuoHDBPd1vtTcsJuIqbQCZ7SPa8BL0Wss4C/Vq98Rp58G9O/yWm4g==" crossorigin="anonymous"></script>
<script src="https://cdn.datatables.net/2.0.1/js/dataTables.bootstrap.min.js" integrity="sha512-VA6ybNLYE2gJREhDvihaesx+djpqYnqOIq8eOVEZPF/lb/Ylur3kIHp46mOVBcyEn6gMPPDGNREsmEnviczMlg==" crossorigin="anonymous"></script>
...
<script type="text/javascript">
    ...
    const defaultBucket = "${bucket_name}";
    const urlParams = new URLSearchParams(window.location.search);
    const hashPrefix = decodeURIComponent(window.location.hash.substring(1)) || '';

    var s3exp_config = {
        Region: "${region}",
        Bucket: urlParams.get("bucket") || defaultBucket,
        CloudFront: '',
        Prefix: hashPrefix,
        Delimiter: '/'
    };
        ...
        return {
            ...
            cb: function(err, data) {
                if (err) {
                    ...
                    bootbox.alert({message: translations['bucketError'][currentLang] + scope.params.Bucket +'. Error: ' + err,});
                } else {
    ...
    bootbox.prompt({
      title: translations['enterBucketName'][currentLang],
      buttons: {
        cancel: {
          label: translations['buttonCancel'][currentLang],
        },
      },
  ...
  const translations = {
    ...
    tablePages: {
      en: 'Showing {start} to {end} of {total} entries',
      fr: 'Affichage {start} à {end} de {total} entrées',
    },
    tablePrevious: {
      en: 'Previous',
      fr: 'Précédent',
    ...
    titleBucketChooser: {
      en: 'Switch to a different S3 Bucket',
      fr: 'Passer à un autre compartiment S3',
    },
    titleRefresh: {
      en: 'Refresh',
      fr: 'Actualiser',
    ...
    folderObjects: {
      en: 'View all objects in folder',
      fr: 'Afficher tous les objets du dossier',
    },
    bucketObjects: {
      en: 'View all objects in bucket',
      fr: 'Afficher tous les objets du compartiment',
    },
    enterBucketName: {
      en: 'Please enter the S3 bucket name',
      fr: 'Veuillez entrer le nom du compartiment S3',
    },
    bucketError: {
      en: 'Error accessing S3 bucket',
      fr: "Erreur lors de l'accès au compartiment S3",
    },
    buttonOk: {
      en: 'Ok',
      fr: 'Ok',
  ...
  function changeLanguage(lang) {
    currentLang = lang;
    //Translate all elements that have id
    for (const [elementId, texts] of Object.entries(translations)) {
    ...
    const titleBucketChooser = document.getElementById('bucket-chooser');
    if (titleBucketChooser) {
      titleBucketChooser.title = translations['titleBucketChooser'][lang];
    }
    const titleRefresh = document.getElementById('bucket-loader');
    if (titleRefresh) {
      titleRefresh.title = translations['titleRefresh'][lang];
    }
    const titleCount = document.getElementById('badgecount');
    if (titleCount) {
      titleCount.title = translations['titleCount'][lang];
    }
    const folderObjects = document.getElementById('folder-objects');
    if (folderObjects) {
      folderObjects.title = translations['folderObjects'][lang];
    }
    const bucketObjects = document.getElementById('bucket-objects');
    if (bucketObjects) {
      bucketObjects.title = translations['bucketObjects'][lang];
    }
  ...
  function translateTable(lang) {
    // Translate table content
    const entriesLabel = document.querySelector('label[for="dt-length-0"]');
    if (entriesLabel) {
      entriesLabel.textContent = translations['tableEntires'][lang];
    }

    const searchLabel = document.querySelector('label[for="dt-search-0"]');
    if (searchLabel) {
      searchLabel.textContent = translations['tableSearch'][lang];
    }
    const infoElement = document.getElementById('tb-s3objects_info');
    if (infoElement) {
      const [start, end, total] = infoElement.textContent.match(/\d+/g) || [];
      infoElement.textContent = translations['tablePages'][lang]
        .replace('{start}', start)
        .replace('{end}', end)
        .replace('{total}', total);
    }

    const previousButton = document.querySelector('#tb-s3objects_previous a');
    if (previousButton) {
      previousButton.textContent = translations['tablePrevious'][lang];
    }

    const nextButton = document.querySelector('#tb-s3objects_next a');
    if (previousButton) {
      nextButton.textContent = translations['tableNext'][lang];
    }

    const tableEmpty = document.querySelector('.dt-empty');
    if (tableEmpty) {
      tableEmpty.textContent = translations['tableEmpty'][lang];
    }
  }
</script>
